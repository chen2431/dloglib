...
void CScrollHelper::ScrollToEnd()
{
	if ( m_displaySize.cy > 0 && m_pageSize.cy > 0 )
	{
		int pos = m_scrollPos.cy;
		m_scrollPos.cy = m_displaySize.cy-m_pageSize.cy;

		m_attachWnd->SetScrollPos(SB_VERT, m_scrollPos.cy, TRUE);

		int deltaPos = pos-m_scrollPos.cy;
		m_attachWnd->ScrollWindow(0, -deltaPos);
	}
}
